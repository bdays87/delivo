...
| 8 | **Cart + checkout** | Login-required cart, address book, currency-aware totals, place order to `PENDING_PAYMENT` against mobile-wallet payment instruction. Per-line tier pricing. Stock decrements on placement; hard-block on stock-out. | ✅ |
...
- **Slice 8 shipped (2026-05-21):** `addresses`/`carts`/`cart_items`/`orders`/`order_items` tables; per-line tier-price computation in cart; address book with default; `POST /api/v1/checkout` produces `DLV-YY-NNNNNN` order in `PENDING_PAYMENT` with payment reference + chosen mobile wallet, decrements variant stock, snapshots line items; hard-block 409 on stock-out. Frontend: `/cart` with qty controls + stock warnings, `/checkout` with address picker + new-address form + wallet picker + live tier-priced totals, `/checkout/instructions/{order}` with copyable reference + ZWG equivalent, `/account/orders` + detail. Header cart badge bound to `useCartStore.itemCount`. Add-to-cart on `/products/[slug]` wired.
- **Slice 8 follow-up shipped (2026-05-21):** added platform service charge (% of subtotal, min-floor) + city-based delivery fee. New tables `platform_settings` (1-row config) + `delivery_zones`; `orders.service_charge_usd` column. `PricingService` centralises the math; reused by `CartService::snapshot` (now returns subtotal + service charge + items total + "delivery calculated at checkout" note) and `CheckoutService::place` (recomputes server-side at place-time, snapshots `service_charge_usd` + `shipping_usd`). New endpoint `POST /api/v1/checkout/quote` returns live breakdown for a selected address. Admin endpoints + UI under `/admin/settings` and `/admin/delivery-zones` (new sidebar submodule). `DeliveryZonesSeeder` seeds 9 Zimbabwean cities (Harare $5 → Vic Falls $12) + default settings (2.5% / $0.50 / $10). Cart + checkout + order-detail UI updated to show the new lines.
- **Next move:** Slice 9 — admin payment confirmation. Admin dashboard surface to mark a `PENDING_PAYMENT` order as `PAID` (with wallet receipt reference); webhook stubs deferred.
