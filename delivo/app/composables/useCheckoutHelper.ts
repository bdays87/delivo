export const useCheckoutHelper = () => {
  ...
  const quoteOrder = async (addressId: number, deliveryMethod: string = 'HOME_DELIVERY') => {
    try {
      const data = await client('/api/v1/checkout/quote', {
        method: 'POST',
        body: { address_id: addressId, delivery_method: deliveryMethod },
      });
  ...
  const placeOrder = async (addressId: number, mobileWalletId: number, deliveryMethod: string = 'HOME_DELIVERY') => {
    try {
